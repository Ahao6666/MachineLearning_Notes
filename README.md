# 机器学习学习笔记

本项目记录了从零开始学习机器学习的过程，包含 Python 基础、Andrew Ng 在 Coursera 上的《Machine Learning Specialization》（机器学习专项课程）三门课程的学习笔记与实验代码，以及一份为期 90 天的机器学习入门学习计划。

## 项目结构

| 目录 | 内容 |
| --- | --- |
| [Python Guidance](./Python%20Guidance) | Python 编程基础与数据可视化入门（NumPy、Pandas、Matplotlib、Seaborn 等） |
| [Coursera1-Supervised Learning](./Coursera1-Supervised%20Learning) | 监督学习：回归与分类 |
| [Coursera2-Advanced Learning Algorithms](./Coursera2-Advanced%20Learning%20Algorithms) | 高级学习算法：神经网络、训练优化、实践建议、决策树 |
| [Coursera3-Unsupervised Learning, Recommenders, Reinforcement Learning](./Coursera3-Unsupervised%20Learning,%20Recommenders,%20Reinforcement%20Learning) | 无监督学习、推荐系统、强化学习 |
| [90天机器学习入门学习计划.md](./90天机器学习入门学习计划.md) | 90 天入门学习路线与每日安排 |

## 1. [Python 入门](./Python%20Guidance)

Python 编程基础与数据可视化入门示例代码，涵盖 NumPy、Pandas、Matplotlib、Seaborn 等常用工具，为后续机器学习课程打下基础。

## 2. [Coursera — 监督机器学习](./Coursera1-Supervised%20Learning)

Coursera 课程《Supervised Machine Learning: Regression and Classification》的学习笔记与实验代码，内容涵盖：

- 监督学习基础概念
- 线性回归与逻辑回归
- 梯度下降与代价函数
- 正则化与过拟合
- 多类分类

详细内容见 [机器学习基础学习笔记](./Coursera1-Supervised%20Learning/机器学习基础学习笔记.md)。

## 3. [Coursera — 高级学习算法](./Coursera2-Advanced%20Learning%20Algorithms)

Coursera 课程《Advanced Learning Algorithms》的学习笔记与四周实验代码，内容涵盖：

- 神经网络基础与前向/反向传播
- 神经网络训练技巧（激活函数、初始化、正则化、优化器等）
- 应用机器学习的建议（偏差/方差、误差分析、学习曲线等）
- 决策树与集成方法

详细内容见 [Advanced_Learning_Algorithms_Notes.md](./Coursera2-Advanced%20Learning%20Algorithms/Advanced_Learning_Algorithms_Notes.md)。

## 4. [Coursera — 无监督学习、推荐系统、强化学习](./Coursera3-Unsupervised%20Learning,%20Recommenders,%20Reinforcement%20Learning)

Coursera 课程《Unsupervised Learning, Recommenders, Reinforcement Learning》的学习笔记与实验代码，内容涵盖：

- **无监督学习**：K-means 聚类、异常检测、高斯分布与密度估计
- **推荐系统**：协同过滤、基于内容的过滤、双塔神经网络、PCA 降维
- **强化学习**：MDP、Q 函数与贝尔曼方程、DQN、经验回放与目标网络

详细内容见 [Unsupervised Learning_Notes.md](./Coursera3-Unsupervised%20Learning,%20Recommenders,%20Reinforcement%20Learning/Unsupervised%20Learning_Notes.md)。

## 学习计划

如果你想按一个相对系统的路线学习，可以参考 [90天机器学习入门学习计划.md](./90天机器学习入门学习计划.md)。该计划覆盖了 Python 基础、三门 Coursera 课程、课后练习与项目实践，为期约 90 天。
